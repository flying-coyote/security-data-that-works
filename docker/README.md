...
| **governance** | `./moar governance`: three commits → three immutable snapshots in an unbroken parent→child chain; time-travel returns the exact state as of each (100/200/300) — the audit trail + lineage a mutable SIEM index can't give (H-SEC-CATALOG-01; Reg SCI / 17a-4(f) evidence) |
...
| **variant-mfa (context-collapse)** | run 2026-06-15: `./moar variant-mfa` is a self-contained DuckDB probe in the lab container (no engine tier) — builds CloudTrail-shaped logins with mfa true/false/ABSENT, then shows the flattened `mfa = 'false'` detection silently under-counts unprotected logins by the absent-MFA population while the nested/structure-aware path catches false + absent. Measured: 820 ConsoleLogin (mfa true 500 / false 120 / absent 200), the naive `mfa = 'false'` detection caught 120 of 320 unprotected logins (it missed all 200 absent-MFA, 62.5%) while the structure-aware path caught all 320 (H-OCSF-CONTEXT-COLLAPSE-01) |
| **flow-gate (flow-layer data-health)** | run 2026-06-15: `./moar flow-gate` runs the count-at-every-hop gate (source tap → OCSF model, per `class_uid`) in the lab container — pure Python, no engine — passing a clean pipeline and catching a silently mis-mapped class 3002 the quality gate (NULLs/timestamps/cross-engine count) can't see. Measured: the clean pipeline reconciles every class at the model boundary (4001→10000, 3002→4000, 1001→2500, 4002→800; READY), while the faulted run drops all 4000 class-3002 events source→model (landed 0; NOT READY) — the whole-class silent drop the quality gate is blind to (book ch03 / Appendix-B #12 / F.2; exit 0 clean, 1 on the drop) |
| **cidr (native-IP CIDR hunting)** | run 2026-06-07, re-run + confirmed 2026-06-15: `./moar cidr` runs `cidr_probe.py`'s SQL in the `engine-clickhouse` server (ClickHouse-internal MergeTree, not the lakehouse). Measured single host (Ryzen 5800H/WSL2, 20M rows, warm): native IPv4 integer range ~0.010 s vs String `isIPAddressInRange` ~0.166 s (~13–17×), storage IPv4 65.4 MiB vs String 188.1 MiB (~2.9×), same 78,211-IP answer in 10.5.0.0/16 — below the borrowed 50–100× headline at this scale; the direction + storage ratio are the durable findings. The 2026-06-15 warm re-run held the result (median ~0.007 s vs ~0.120 s ≈ 17×, storage 69.21 vs 188.10 MiB ≈ 2.72×, 78,222-IP answer); the 2026-06-15 re-run is the canonical set. |
| **commit-tax (write-pattern tax)** | run 2026-06-15: `./moar commit-tax` ingests the same rows as one batch commit vs N streaming commits into Iceberg and DuckLake (inlining off, then on) on the real MinIO store, in the lab container, and reports the metadata footprint + planning cost — the file-per-commit metadata floor Iceberg pays vs DuckLake's catalog-DB metadata + small-commit inlining. Measured (100k rows, Iceberg batch → 100-commit stream): metadata files 4→301 (75×), metadata size 8.8→4577.1 KB (520×), planning 9.9→142.5 ms (14.4×); DuckLake holds commit metadata in the catalog DB, so its streamed planning stays 5.6–7.2 ms (~20–25× faster than Iceberg's streamed 142.5 ms) and inlining-on writes 0 Parquet files for the small commits (H-DUCKLAKE-02; the shape of the tax is the finding, single host) |
| **concurrency-sweep (engine scheduling model)** | run 2026-06-15: `./moar concurrency-sweep` drives C concurrent clients (C = 1,2,4,8,16) running the same scan-aggregate over the shared `ocsf.network_activity_bench`, reporting throughput (q/s) + p50/p95 per engine as C rises — the embedded engine (DuckDB, one in-process engine per client) vs the server engines (ClickHouse/StarRocks/Trino, C connections to one scheduler); each engine skipped if its tier isn't up. Measured (1M rows, scan-agg group-by, C16/C1 throughput scaling): embedded DuckDB flattens at 1.52× with its p95 climbing hardest (+8.4×), while the server schedulers grow throughput — ClickHouse 3.71×, Trino 4.00×, StarRocks 5.35× (the other half of H-ARCH-02 the sequential bench only hinted at; single host = the scheduling model, not a cluster) |
