...
| **variant-mfa (context-collapse)** | wired — run pending: `./moar variant-mfa` is a self-contained DuckDB probe in the lab container (no engine tier) — builds CloudTrail-shaped logins with mfa true/false/ABSENT, then shows the flattened `mfa = 'false'` detection silently under-counts unprotected logins by the absent-MFA population while the nested/structure-aware path catches false + absent (H-OCSF-CONTEXT-COLLAPSE-01) |
| **flow-gate (flow-layer data-health)** | wired — run pending: `./moar flow-gate` runs the count-at-every-hop gate (source tap → OCSF model, per `class_uid`) in the lab container — pure Python, no engine — passing a clean pipeline and catching a silently mis-mapped class 3002 the quality gate (NULLs/timestamps/cross-engine count) can't see (book ch03 / Appendix-B #12 / F.2; exit 0 demonstrated, 1 otherwise) |
| **cidr (native-IP CIDR hunting)** | wired; recorded 2026-06-07, scored re-run pending: `./moar cidr` runs `cidr_probe.py`'s SQL in the `engine-clickhouse` server (ClickHouse-internal MergeTree, not the lakehouse). Measured single host (Ryzen 5800H/WSL2, 20M rows, warm): native IPv4 integer range ~0.010 s vs String `isIPAddressInRange` ~0.166 s (~13–17×), storage IPv4 65.4 MiB vs String 188.1 MiB (~2.9×), same 78,211-IP answer in 10.5.0.0/16 — below the borrowed 50–100× headline at this scale; the direction + storage ratio are the durable findings |
| **commit-tax (write-pattern tax)** | wired — run pending: `./moar commit-tax` ingests the same rows as one batch commit vs N streaming commits into Iceberg and DuckLake (inlining off, then on) on the real MinIO store, in the lab container, and reports the metadata footprint + planning cost — the file-per-commit metadata floor Iceberg pays vs DuckLake's catalog-DB metadata + small-commit inlining (H-DUCKLAKE-02; the shape of the tax is the finding, single host) |
| **concurrency-sweep (engine scheduling model)** | wired — run pending: `./moar concurrency-sweep` drives C concurrent clients (C = 1,2,4,8,16) running the same scan-aggregate over the shared `ocsf.network_activity_bench`, reporting throughput (q/s) + p50/p95 per engine as C rises — the embedded engine (DuckDB, one in-process engine per client) vs the server engines (ClickHouse/StarRocks/Trino, C connections to one scheduler); each engine skipped if its tier isn't up (the other half of H-ARCH-02 the sequential bench only hinted at; single host = the scheduling model, not a cluster) |
